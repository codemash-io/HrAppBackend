...
namespace LambdaFunction
{
    public class Function
    {
        ...
        /// <returns></returns>
        public async Task<List<DriveItem>> Handler(CustomEventRequest<BasicInput> lambdaEvent, ILambdaContext context)
        {
            string type, typeId, itemId, path, expand, select, top, skipToken, orderBy;
            ...
            else
            {
                ...
                skipToken = Environment.GetEnvironmentVariable("skipToken");
                orderBy = Environment.GetEnvironmentVariable("orderBy");
            ...
            var children = await graphFileRepo.ListChildren(type, typeId, itemId, path,
                expand, select, skipToken, top, orderBy);
