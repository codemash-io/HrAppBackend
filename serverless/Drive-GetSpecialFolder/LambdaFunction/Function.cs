...
namespace LambdaFunction
{
    public class Function
    {
        ...
        /// <returns></returns>
        public async Task<DriveItem> Handler(CustomEventRequest<BasicInput> lambdaEvent, ILambdaContext context)
        {
            string type, typeId, folderName;
            ...
            if (string.IsNullOrEmpty(type) || string.IsNullOrEmpty(folderName))
                throw new BusinessException("All fields is required!");

            IGraphFileRepository graphFileRepo = new GraphFileRepository();
            var specialFolder = await graphFileRepo.GetSpecialFolder(type, folderName, typeId);
