...
namespace LambdaFunction
{
    public class Function
    {
        ...
        {
            string type, typeId, itemId, size, thumbId;
            ...
            if (string.IsNullOrEmpty(type) || string.IsNullOrEmpty(itemId) || string.IsNullOrEmpty(size)
                || string.IsNullOrEmpty(thumbId))
                throw new BusinessException("Type, itemId, size, thumbnailId is required!");

            IGraphFileRepository graphFileRepo = new GraphFileRepository();
            var thumb = await graphFileRepo.GetSingleThumbnailContent(type, itemId, thumbId, size, typeId);
